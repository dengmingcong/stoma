...
"""
        ...
        # datamodel-codegen 包装 array-of-ref response 时按 method+path 派生
        # （``GET /users`` → ``UsersGetResponse``），renderer 同步引用同名。
        assert "APIRoute[UsersGetResponse]" in content
        assert "from .models import UsersGetResponse" in content
...
"""
        ...
        # 嵌套对象响应也按 method+path 派生模型名（``GET /profile`` →
        # ``ProfileGetResponse``），从 .models 导入。
        assert "from .models import ProfileGetResponse" in content
        assert "APIRoute[ProfileGetResponse]" in content
