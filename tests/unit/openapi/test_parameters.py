...
class TestParamFieldDocstring:
    """参数（header / query / path）字段 docstring 端到端渲染测试。

    每类参数 × 4 种 docstring 形态：
    ...
    @staticmethod
    def _build_spec(field_block: str) -> str:
        """构造带自定义参数的 OpenAPI 3.1 模板（参数块由 ``field_block`` 注入）。"""
        return f"""\
openapi: 3.1.0
info:
  title: Docstring API
...
"""
    ...
    def test_description_only_renders_single_line_docstring(
        ...
        assert field_docstring_token in content, content

    @pytest.mark.parametrize(
        ("field_block", "description_token", "example_token"),
        [
            (
                "        - name: query_key\n"
                "          in: query\n"
    ...
    def test_description_and_single_example_renders_multiline_docstring(
        ...
        """每类参数有 description + 1 个 example 时，docstring 是多行三引号，且含 ``Example: <repr>``。"""
        spec = self._build_spec(field_block)
        content = self._run(cli_runner, spec, tmp_path / "out")
    ...
    @pytest.mark.parametrize(
        ("field_block", "description_token", "bullet_token"),
        [
            (
                "        - name: status_code\n"
                "          in: query\n"
    ...
    def test_description_and_multiple_examples_renders_bullet_list(
        ...
        """每类参数有 description + 多个 examples 时，docstring 是多行 + 项目符号列表。"""
        spec = self._build_spec(field_block)
        content = self._run(cli_runner, spec, tmp_path / "out")
        assert description_token in content
        assert "Examples:" in content
        assert bullet_token in content

    @pytest.mark.parametrize(
        ("field_block", "field_line_token"),
        [
            (
                "        - name: q\n"
                "          in: query\n"
    ...
    def test_no_description_no_example_no_docstring_line(
        ...
        """每类参数既无 description 也无 example 时，docstring 为 None，模板条件跳过。"""
        spec = self._build_spec(field_block)
        content = self._run(cli_runner, spec, tmp_path / "out")
        assert field_line_token in content
        compile(content, "get_item.py", "exec")
...
class TestNullableParameterSchemas:
    ...
    def test_opPlainScalar_param(self) -> None:
        ...
        rendered = "\n".join(decl.line for decl in param_fields)
