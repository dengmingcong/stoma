...
class Client:
    ...
    def send[T](self, api_route: APIRoute[T]) -> Response[T]:
        """发送 api_route 请求，返回 :class:`Response[T]`。

        T 通过 PEP 695 泛型方法从 api_route 的类型参数自动推断。

        请求构造委托 :func:`src.dependencies.request.build_request`，
        响应构造委托 :func:`src.dependencies.response.build_response`。
        ...
        try:
            ...
            return build_response(api_route, api_response)
