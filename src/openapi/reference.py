"""OpenAPI 规范中 ``$ref`` 引用的解析与校验。

集中处理：

- 循环检测：沿 ``components.parameters`` 表展开 ``$ref`` 链，遇到环立即报错，
  避免下游 :mod:`jsonref` 陷入无限递归。
- 选择性展开：在 ``raw_spec`` 的 ``paths[*]`` 操作级 + path item 级
  ``parameters`` 与 ``requestBody`` 上就地展开 ``$ref``；其余字段（``responses``
  、``summary``、``description`` 等）保持原样不动，``requestBody`` 的展开结果
  抽离到 ``request_body_map`` 中交给 renderer 单独处理。
"""
...
PARAMETER_REF_PREFIX: str = "#/components/parameters/"


def validate_cycle_refs(raw_spec: dict[str, Any]) -> None:
    ...
    :raise OpenAPISchemaError: ``components.parameters`` 中的 ``$ref`` 链构成环；
        错误信息包含形如 ``"A -> B -> A"`` 的路径字符串（起点参数名闭合）。
    """
